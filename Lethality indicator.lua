...
local is_leth_flag, is_force_flag, is_icon, is_forcing
...
--#region references
local ref = {
	min_dmg = _ui.reference('rage', 'aimbot', 'minimum damage'),
	min_dmg_ovr = {_ui.reference('rage', 'aimbot', 'minimum damage override')},
	dpi = _ui.reference('misc', 'settings', 'dpi scale'),

	name = {_ui.reference('visuals', 'player esp', 'name')},
	weapon_text = {_ui.reference('visuals', 'player esp', 'weapon text')},
	weapon_icon = { _ui.reference('visuals', 'player esp', 'weapon icon')}
}
local show_nades
...
local callbacks = {
	setup_command = function(cmd)
		local lp = get_local_player(); if lp == nil or (lp:has_short_range_weapon() or not lp:is_alive()) then lethal.targets = {} return end
		if cmd.chokedcommands > 0 then return end

		lethal.targets = {}
		
		local min_dmg = ref.min_dmg()
		if ref.min_dmg_ovr[1]() and ref.min_dmg_ovr[2]() then min_dmg = ref.min_dmg_ovr[3]() end
		...
		player_iterator('ADE', function(ent)
			...
			for orb, origin in ipairs(orbs) do
				...
				if c_dmg ~= nil and (c_dmg > min_dmg or c_dmg > hp) then lethal.targets[idx].dmg = c_dmg end
			end
	...
	paint = function()
		...
		local icon_pos = feature.move_icon()

		dpi = tonumber( ref.dpi():sub(1, 3) ) * .01

		if is_icon then
			for _, i in pairs(lethal.targets) do
				if i.entity == nil or i.entity:is_alive() == false or i.dmg <= 0 then goto continue end

				local bb = {i.entity:get_bounding_box()}; if bb[1] == nil then goto continue end
				...
				if icon_pos == 'Default' then
					...
					if icon_pos == 'Above name' then
						if show_nades and ui_get(show_nades) then offset = offset+10 end
						if ref.name[1]() then offset = offset+10 end
					elseif icon_pos == 'Below weapons' then
						offset = 8
						if ref.weapon_icon[1]() then offset = offset+15 end
						if ref.weapon_text[1]() then offset = offset+8 end
					end

					local positions = {
						['Above name'] = { (bb[1]+bb[3])*.5-8*dpi, bb[2]-offset*dpi },
						['Below weapons'] = { (bb[1]+bb[3])*.5-8*dpi, bb[4]+offset*dpi },
					...
					pos.x, pos.y = unpack(positions[icon_pos])
				end
...
register_flag('-', 255, 0, 0, function(e)
	if (not feature.enabled()) or not is_leth_flag then return false end
...
register_flag('B', 255, 255, 255, function(e)
	if (not feature.enabled()) or not is_force_flag then return false end

	return plist_get(e, 'Override prefer body aim') == 'Force'
end)
--#endregion
...
local ui_vis = function()
	...
	is_leth_flag, is_force_flag, is_icon, is_forcing =
		feature.options:contains('Lethal flag'), feature.options:contains('Force flag'), feature.options:contains('Bullet icon'), feature.options:contains('Force body aim')
